describe('Showing a profession', () => {
  ...
  it('I can view a profession with the bare minimum fields', () => {
    ...
    ).then((otherCountriesRecognition) => {
      cy.get('body').should('not.contain', otherCountriesRecognition);
    });
    cy.translate(
      'professions.show.qualification.otherCountriesRecognitionUrl',
    ).then((otherCountriesRecognitionUrl) => {
      cy.get('body').should('not.contain', otherCountriesRecognitionUrl);
    });
