...
const argv = yargs(hideBin(process.argv)).argv as { chunks?: number, matrix?: number };

if (typeof argv.chunks !== 'number' || typeof argv.matrix !== 'number') {
  throw new Error('Both chunks and matrix arguments must be provided and must be numbers.');
}
...
glob('./cypress/integration/**/*.spec.ts', {}, (_er, files) => {
  ...
  for (let remainingChunks = numberOfChunks; remainingChunks > 0; remainingChunks--) {
