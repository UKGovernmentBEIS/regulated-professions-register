describe('Listing professions', () => {
  context('When I am logged in as admin', () => {
    beforeEach(() => {
      cy.loginAuth0('admin');
      cy.visit('/admin/professions/list');

      cy.translate('professions-list.showFilters').then((showFilters) => {
    ...
    it('The list page contains the expected columns', () => {
      cy.translate('professions-list.tableHeading.organisation').then(
        (organisation) => {
          cy.get('tr').eq(0).should('contain', organisation);
        },
      );

      cy.translate('professions-list.tableHeading.industry').then(
        (industry) => {
          cy.get('tr').eq(0).should('contain', industry);
        },
      );

      cy.translate('professions-list.tableHeading.changedBy').then(
        (changedBy) => {
  ...
  context('When I am logged in as editor', () => {
    ...
    it('The list page contains the expected columns', () => {
      cy.translate('professions-list.tableHeading.regulators').then(
        (regulators) => {
...
function clickFilterButton(): void {
  cy.translate('professions-list.form.button.filter').then((buttonLabel) => {
